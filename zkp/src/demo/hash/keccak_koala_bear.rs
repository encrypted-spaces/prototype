...
pub fn keccak_koala_bear(batch_size: usize) -> Vec<u8> {
    type Val = KoalaBear;
    ...
    let inputs = (0..batch_size).map(|_| rng.random()).collect::<Vec<_>>();
    ...
    verify(&config, &KeccakAir {}, &proof, &[]).unwrap();

    serialized
}

pub fn keccak_koala_bear_zk(batch_size: usize) -> Vec<u8> {
    type Val = KoalaBear;
    type Challenge = BinomialExtensionField<Val, 4>;
