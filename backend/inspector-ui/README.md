...
| **Backend on its own** — useful for loading a previously captured `.ndjson`, driving the backend with the test harness or your own SDK code, or debugging the backend without the Tauri client. | `cargo run -p cypherspaces-backend-server` | `http://localhost:8080/_inspect/` |
...
`CYPHERSPACES_INSPECTOR_LOG` before starting the backend to enable both
...
```sh
CYPHERSPACES_INSPECTOR_LOG=/tmp/inspector.ndjson \
  cargo test --package cypherspaces-demo-test-harness \
...
cypherspaces-backend-server` embeds. If `dist/` is missing at build time
