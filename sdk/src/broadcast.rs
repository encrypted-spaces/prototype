        ...
        loop {
            match rx.recv().await {
                Ok(evt) => {
                    ...
                    drop(space);
                    // Only forward to app subscribers when the broadcast was
                    // actually applied. Rejected frames (bad signature, etc.)
                    // must not reach `subscribe_updates()` consumers.
                    if applied {
...
impl Space {
    ...
    ///
    /// Returns `true` when the broadcast was applied (local state changed and
    /// subscribers should refresh), and `false` when it was skipped or rejected
    /// (stale, invalid signature, failed deferred-sig retry, fast-forward, or
    /// apply error). Callers must not forward a rejected event to app consumers.
